...
import { StudentFavorite } from '../../types/database';
...
interface StudentFavoritesFormProps {
  user: any;
}

function StudentFavoritesForm({ user }: StudentFavoritesFormProps) {
  const [searchQuery, setSearchQuery] = useState('');
  const [filterCategory, setFilterCategory] = useState('all');
  const [viewMode, setViewMode] = useState<'grid' | 'list'>('grid');
  const [showAddFavorite, setShowAddFavorite] = useState(false);
  const [loading, setLoading] = useState(true);
  const [favoriteBooks, setFavoriteBooks] = useState<StudentFavorite[]>([]);
    ...
    const loadFavorites = async () => {
      try {
        ...
        if (user && user.id && user.role === 'student') {
          ...
          const favorites = await databaseService.getStudentFavorites(user.id);
  ...
  const handleAddFavorite = async (bookId: string) => {
    try {
      if (user && user.id) {
        const newFavoriteData = await databaseService.addStudentFavorite(
          user.id,
          bookId,
          {
          ...
          <div key={favorite.id} className="border border-gray-200 rounded-lg overflow-hidden hover:shadow-lg transition-shadow">
            <div className="relative">
              ...
              <div className="absolute top-2 right-2">
                <span className={`px-2 py-1 rounded-full text-xs font-medium ${getAvailabilityColor(book.status)}`}>
                  {book.status === 'available' ? 'Tersedia' : 
                   book.status === 'borrowed' ? 'Dipinjam' : 'Direservasi'}
                </span>
              </div>
              <button
            ...
            <div className="p-4">
              ...
              <div className="flex space-x-2">
                <button className="flex-1 px-3 py-2 bg-blue-600 text-white rounded hover:bg-blue-700 transition-colors text-sm">
                  ...
                  {book.status === 'available' ? 'Pinjam' : 'Reservasi'}
                </button>
